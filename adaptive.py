...
def my_loss_only1(ro):
    def my_loss(y_true,y_pred):
        ...
        y5=tf.math.log(y22)
        y6= tf.reduce_max(y_pred, axis=1)
        y6=tf.reshape(y6,[-1,1])
        y7=tf.math.log(y6)       
        loss = -(ro * y5) - (1.0 - ro)*y7
        cost = K.mean(loss)
        return cost
    return my_loss 
####################################base-model#############
...
merged_model1.compile(loss=my_loss_only1(ro=0.85) , optimizer= 'adam',metrics=['binary_accuracy'] )
j=0
i=0
...
# Fit the model
def modelf3():
	m3=merged_model1.fit([X_train1, X_train2], y_train, nb_epoch=1, batch_size=100, verbose=2, shuffle=True)
	return m3

def modele3():
	e3=merged_model1.evaluate([X_test1,X_test2],y_test, verbose=0)
	return e3

def modelp3():
	p3=merged_model1.predict([X_test1, X_test2])
	return p3
